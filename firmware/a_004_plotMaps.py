...
for currentNode in nodeIDs:
    ...
    if os.path.exists(file_path):
        ...
        # print(mintsDefinitions['vars'])
        # Create a feature group for the markers
        print("TEST 2")
        
        for rowVar in mintsDefinitions['vars']:
            m = folium.Map(location=map_center, zoom_start=20)
            print("TEST 3")
        
            varUsed = rowVar['ID']
            varLabel = rowVar['Label']
            print("========================NODES========================")

            print("Reading node data for "+ nodeName+ " with node ID " + nodeID + " for " + varUsed + " "+ varLabel)
            print("TEST 4")
            marker_group = folium.FeatureGroup(name= varLabel)
            ...
            file_path = dataFolderParent + "/plots/surveplots/" + nodeID +  "/" +nodeID + "_" + nodeName + "_" + varUsed + "_"+ startTime +"-" +endTime + '_surveyPlot_' +resampleTime+'.html'
            directory_path = os.path.dirname(file_path)
